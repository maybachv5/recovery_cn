...
int show_install_update_menu()
{
    ...
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes + 1] = "从sideload读取刷机包";
...
int confirm_selection(const char* title, const char* confirm)
{
    ...
    const char* confirm_headers[]  = {  title, "确认后将不能取消.", "", NULL };
...
void show_nandroid_advanced_restore_menu(const char* path)
{
    ...
    static char* confirm_restore  = "确定还原备份?";
    ...
    for (;;)
    {
        ...
        switch (chosen_item)
        {
            ...
            case 3:
                ...
                if (confirm_selection( "确定要清除?", "是-清除Dalvik Cache")) {
...
void handle_failure(int ret)
{
    ...
    ui_print("/tmp/recovery.log已复制到/sdcard/clockworkmod/recovery.log. 请打开ROM Manager来发送此报告.\n");
}
...
int verify_root_and_recovery() {
    ...
    if (0 == lstat("/system/bin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            if ((st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_show_text(1);
                ret = 1;
                if (confirm_selection("是否修复root权限?", "是-修复(/system/bin/su)")) {
    ...
    if (0 == lstat("/system/xbin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            if ((st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_show_text(1);
                ret = 1;
                if (confirm_selection("是否修复root权限?", "是-修复(/system/xbin/su)")) {
    ...
    if (!exists) {
        ui_show_text(1);
        ret = 1;
        if (confirm_selection("设备尚未获取root权限,是否获取?", "是,安装(/system/xbin/su)")) {
